...
class KeyPad(_74HC165):
	...
	def __update_kb_data2(self):
		'''自定义发送内容'''
		time.sleep(0.1)
		# 定义字符对应的键码
		chr = [KC.H, KC.NONE, KC.E, KC.NONE, KC.L, KC.NONE, KC.L, KC.NONE, KC.O, KC.NONE]
		# 计算索引，使用 self._pi % 10
		idx = self._pi % 10
		keyCode = chr[idx]
		status = 1
		...
		# 获取对应的键码
		# 设置键码状态为按下
		self.__set_kb_data(keyCode, status)
		self._pi = self._pi + 1
	...
	def send_selfdefine_data(self):
		self.__update_kb_data2()
		self.__ble_keyboard.send_kb_key(
			bytearray([self.__kb_modifier, 0x00]) + self.__kb_data_buffer)
