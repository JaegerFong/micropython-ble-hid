...
class BLEVolumeKey(object):
    ...
    def __init__(
        ...
        self.__handle_reports = [
            None for _ in range((keyboard_profile.report_count + 1) * 2)
        ]
        self.__handle_report_references = []

        self.__register_services(
            generic_profile.get_services() + keyboard_profile.get_services()
        )
        ...
        resp_payload = BLETools.generate_advertising_payload(
            keyboard_profile.get_services_uuid(), for_resp=True
        )
    ...
    def __register_services(self, services: list):
        ...
        if False:
            ...
            printf(
                f"  - report_reference_input:",
                self.__handle_report_references[0],
                self.__hid_values.human_interface_device.report_reference[0],
            )

            for index in range(1, self.__report_count):
                printf(f"- report_{index}:", self.__handle_reports[index])
                printf(
                    f"  - report_reference_input:",
                    self.__handle_report_references[index],
    ...
    def __advertise(
        self,
        adv_payload: bytes = None,
        resp_payload: bytes = None,
    ...
    def __irq_callback(self, event, data):
        if event == IRQ.CENTRAL_CONNECT:
            ...
            printf(
                f"[{BLETools.decode_mac(addr)}] Disconnected [Handle: {conn_handle}]"
            )
            ...
            printf(
                f"GATTC Indicate [Handle: {conn_handle}, Value_Handle: {value_handle}, Data: {bytes(data)}]"
            )
            ...
            if conn_handle != 0xFFFF:
                printf(
                    f"GATTS Read Request [Handle: {conn_handle}, Attr_Handle: {attr_handle}]"
                )
            ...
            printf(
                f"Connection Update [Handle: {conn_handle}, Interval: {interval}, Latency: {latency}, Supervision_Timeout: {supervision_timeout}, Status: {status}]"
            )
        elif event == IRQ.ENCRYPTION_UPDATE:
            conn_handle, encrypted, authenticated, bonded, key_size = data
            printf(
                f"Encryption Update [Handle: {conn_handle}, Encrypted: {bool(encrypted)}, Authenticated: {bool(authenticated)}, Bonded: {bool(bonded)}, Key_Size: {key_size}]"
            )
            ...
            printf(
                f"GATTS Indicate Done [Handle: {conn_handle}, Value_Handle: {value_handle}, Status: {status}]"
            )
    ...
    def __setup_hid_values(self):
        ...
        self.__write(
            self.__handle_device_name, self.__generic_values.generic_access.device_name
        )
        self.__write(
            self.__handle_appearance, self.__generic_values.generic_access.appearance
        )
        self.__write(self.__handle_ppcp, self.__generic_values.generic_access.ppcp)

        # DeviceInformation values
        self.__hid_values.device_information.model_number = "MP_VOLUME"

        self.__write(
            self.__handle_manufacturer_name,
            self.__hid_values.device_information.manufacturer_name,
        )
        self.__write(
            self.__handle_model_number,
            self.__hid_values.device_information.model_number,
        )
        self.__write(
            self.__handle_serial_number,
            self.__hid_values.device_information.serial_number,
        )
        self.__write(
            self.__handle_hardware_revision,
            self.__hid_values.device_information.hardware_revision,
        )
        self.__write(
            self.__handle_firmware_revision,
            self.__hid_values.device_information.firmware_revision,
        )
        self.__write(
            self.__handle_software_revision,
            self.__hid_values.device_information.software_revision,
        )
        ...
        self.__write(
            self.__handle_battery_level, self.__hid_values.battery_service.battery_level
        )
        ...
        self.__write(
            self.__handle_hid_information,
            self.__hid_values.human_interface_device.hid_information,
        )
        self.__write(self.__handle_report_map, bytes(self.__report_map))
        self.__write(
            self.__handle_protocol_mode,
            self.__hid_values.human_interface_device.protocol_mode,
        )
    ...
    def send_volume_up_1(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[0], bytearray([self.REPORT_1_VOL_UP]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[0])

    def send_volume_down_1(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[0], bytearray([self.REPORT_1_VOL_DOWN]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[0])

    def send_volume_release_1(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[0], bytearray([0]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[0])

    def send_volume_up_2(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[1], bytearray([self.REPORT_2_VOL_UP]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[1])

    def send_volume_down_2(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[1], bytearray([self.REPORT_2_VOL_DOWN]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[1])

    def send_volume_release_2(self):
        if self.__conn_handles is None:
            return

        self.__write(self.__handle_reports[1], bytearray([0]))

        for conn_handle in self.__conn_handles:
            self.__notify(conn_handle, self.__handle_reports[1])

    def send_volume(self, key_data: bytes | bytearray, report_id: int = 0):
        if self.__conn_handles is None:
            return
